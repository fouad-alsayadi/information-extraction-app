...
UPLOAD_BASE_PATH = os.getenv('UPLOAD_BASE_PATH', '/Volumes/fouad_demos/portfolio_analysis/information_extraction_uploads/')
...
async def create_job(job: ExtractionJobCreate):
    ...
    try:
        ...
        job_id = create_extraction_job(db_job)
                ...
                if current_stage == 'PENDING':
                    ...
                    if result_state == 'SUCCESS':
                        ...
                        if job.status != 'failed':
                            error_msg = databricks_status.get('state', {}).get('state_message', 'Processing failed')
                            update_extraction_job(job_id, {
                                'status': 'failed',
